...
export default function Home() {
  const [profile, setProfile] = useState(() => {
    try {
      ...
      return raw ? { ...initialProfile, ...JSON.parse(raw) } : initialProfile
    } catch {
  ...
  useEffect(()=>{
    const id = setTimeout(()=>localStorage.setItem(LS_KEY, JSON.stringify(profile)), 250)
    return ()=>clearTimeout(id)
  }, [profile])

  const scores = useMemo(()=>{
    let energy = 50
    energy += (sleepHours - 7) * 6
    const age = Number(profile.age || 35)
    if (age < 30) energy += 4
    else if (age >= 55) energy -= 4
    if (['hot','cold'].includes(weather)) energy -= 4
    if (weather === 'sunny') energy += 3

    let moodScore = 45 + (mood - 3) * 10
    if (weather === 'sunny') moodScore += 4
    if (weather === 'rain') moodScore -= profile.indoor_on_rain ? 1 : 3

    let social = 40
    social += profile.relationship === 'single' ? 6 : 2
    if (timeBudget >= 90) social += 8
    else if (timeBudget >= 45) social += 4
    if (profile.chronotype === 'evening') social += 3

    let stress = 40
    if (sleepHours < 6) stress += 12
    if (timeBudget < 45) stress += 8
    if (['hot','wind'].includes(weather)) stress += 4
  ...
  const windows = useMemo(()=>{
    const res=[]
    const push=(label,Icon)=>res.push({label,Icon})
    if (profile.chronotype === 'morning'){
      push('08:30–10:30 • Focus/commissioni', Sun)
      push('18:00–20:00 • Social leggero', CloudSun)
    } else if (profile.chronotype === 'evening'){
      push('10:00–12:00 • Focus', Sun)
      push('19:00–21:30 • Social/tempo libero', Moon)
    } else {
      push('10:00–12:00 • Focus', Sun)
      push('17:30–19:30 • Social leggero', CloudSun)
    }
    return res
  }, [profile.chronotype])
  ...
  const tips = useMemo(()=>{
    ...
    if (scores.social >= 60 && timeBudget >= 60){
      if (weather === 'rain' && profile.indoor_on_rain){
        arr.push({ title:'Serata sociale indoor', reason:'Socialità alta + pioggia', action:`Cinema o aperitivo in ${profile.city || 'zona'} (budget €${moneyBudget}).`, Icon:Heart })
      } else {
        arr.push({ title:'Invito last-minute', reason:'Hai tempo e voglia', action:'Scrivi a 2 amici per un caffè o passeggiata al tramonto.', Icon:Heart })
      }
    }
    if (scores.energy >= 55){
      arr.push({ title:"30' movimento leggero", reason:'Energia buona', action: weather==='sunny' ? 'Camminata all’aperto' : 'Stretching a casa', Icon:Activity })
    } else {
      arr.push({ title:'Recovery intelligente', reason:'Energia bassa', action:"Idratazione + 10' respirazione 4-7-8 + a letto 30' prima.", Icon:Moon })
    }
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '16px 12px' }}>
      {/* HEADER: va a capo su mobile (cols===1) */}
      <header
        style={{
          display:'flex',
          flexDirection: cols===1 ? 'column' : 'row',
          alignItems: cols===1 ? 'flex-start' : 'center',
          justifyContent: cols===1 ? 'flex-start' : 'space-between',
          gap: 8,
          marginBottom: 16
        }}
      >
        <div style={{ display:'flex', alignItems:'center', gap:8, width: cols===1 ? '100%' : 'auto' }}>
          <div style={{ padding:8, borderRadius:16, background:'#4f46e5', color:'#fff' }}>
            <Sparkles size={18}/>
          </div>
          <h1 style={{ fontSize: 22, fontWeight: 700, margin: 0 }}>Oroscopo Data-Driven</h1>
        </div>
        ...
        <div
          style={{
            ...
            gap:8,
            alignItems:'center',
            marginTop: cols===1 ? 8 : 0,
            width: cols===1 ? '100%' : 'auto',
            flexWrap:'wrap',
            justifyContent: cols===1 ? 'flex-start' : 'flex-end'
          }}
        >
          {sessionEmail ? (
            <span style={{ fontSize:12 }}>Connesso: {sessionEmail}</span>
          ) : (
            <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
              <input
                ...
                style={{ padding:'8px 10px', border:'1px solid #d1d5db', borderRadius:12, minWidth: 200 }}
              />
              <button
                onClick={signInWithEmail}
                style={{ padding:'8px 12px', borderRadius:12, background:'#4f46e5', color:'#fff' }}
              >
                Accedi
              </button>
            </div>
          ...
          <button
            ...
            style={{ padding:'8px 12px', borderRadius:12, border:'1px solid #d1d5db', background:'#fff' }}
          >
            Profilo
          </button>
        </div>
